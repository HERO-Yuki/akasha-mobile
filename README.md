...
- **Auto Refresh**（Sideloadly左下の ↻ ボタン）を有効にすると自動で再署名される。
  ただし **2026-08-31 の実測では Wi-Fi 経由は動かなかった**（Apple側は正常で iPhone は
  Bonjour で広告を出しているのに、Sideloadly が接続しに行かない。daemonログ全期間で
  `WiFi:true` は0回）。**USB接続しているときにだけ再署名される**前提で運用する
- 自動再署名の予定時刻を過ぎてから期限までに**一度挿せばその場で再署名される**（充電ついででよい）
